...
void SPI_init() {
	...
	xil_printf("in spi after config...\n");
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	// two writes -> command byte and data byte
	...
	//XSpi_Transfer( XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount );
	rc = XSpi_Transfer( &SpiInstance, writeBuffer, NULL, 2 );
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	rc = XSpi_Transfer( &SpiInstance, writeBuffer, NULL, nbytes + 1 ); //note
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//XSpi_Transfer( XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount );
	rc = XSpi_Transfer( &SpiInstance, writeBuffer, readBuffer, 2 );
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//XSpi_Transfer( XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount );
	rc = XSpi_Transfer( &SpiInstance, writeBuffer, readBuffer, nbytes + 1 ); // check nbytes if error
	if( rc != 0 )
	{
		xil_printf( "Error" );
	}
