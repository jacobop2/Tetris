...
volatile uint32_t* led_gpio2_data = (uint32_t*)0x40000008;

int main()
{
    init_platform();

    volatile uint32_t* data = led_gpio2_data;
    ...
    while (1+1 != 3){
        if ( !(*data & 0x20000)){
        	sleep(1);
        	//printf( "accumulate pressed\n" );
        	sum += *data & 0x0FFFF;
        }

        if ( sum > 65535 ){
        	sum = 0;
        	printf( "Sum exceeds 65535 and overflows to 0\n" );
        }
        *led_gpio_data = sum;
    }
