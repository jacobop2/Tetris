...
//    assign raddr4 = 8'd201;
    ...
    logic Z_block_state = 2'b00; //start state of block I up and down
    ...
    always_ff @ ( posedge frame_clk ) 
        ...
        if ( GenerateBlock == 1'b1 ) begin
            ...
            if ( frame_counter == FRAME_COUNTER_MAX ) begin
                ...
                if ( BlockY + BLOCK_SIZE + HEIGHT <= GRID_END_Y && ( Z_block_state == 2'b11 || Z_block_state == 2'b01 ) && ( board_regs[board_reg_idx + 30][5] == 1'b0 ) 
                && ( board_regs[board_reg_idx + 21][5] == 1'b0 ) ) begin 
                    BlockY <= BlockY + BLOCK_SIZE;
                end 
                // In horizontal
                else if ( ( BlockY + BLOCK_SIZE + HEIGHT <= GRID_END_Y ) && ( Z_block_state == 2'b00 || Z_block_state == 2'b10 ) && ( board_regs[board_reg_idx + 10][5] == 1'b0 )
        ...
        if ( frame_counter_xmove == FRAME_COUNTER_XMOVE_MAX && ( ( BlockY + HEIGHT + BLOCK_SIZE <= GRID_END_Y && ( Z_block_state == 2'b11 || Z_block_state == 2'b01 ) ) 
                                                        || ( BlockY + WIDTH + BLOCK_SIZE <= GRID_END_Y && ( Z_block_state == 2'b10 || Z_block_state == 2'b00 ) ) ) ) begin //10
            
             //***** MOVE *****
            // If A is pressed and that move is in bounds
            if ( aPressed == 1'b1 && ( BlockX - BLOCK_SIZE ) >= GRID_START_X &&  ( Z_block_state == 2'b11 || Z_block_state == 2'b01 ) && ( board_regs[board_reg_idx][5] == 1'b0 )
            && ( board_regs[board_reg_idx + 9][5] == 1'b0 ) && ( board_regs[board_reg_idx + 19][5] == 1'b0 ) ) begin                               
                BlockX <= BlockX - BLOCK_SIZE;
            end
            
            else if ( aPressed == 1'b1 && ( BlockX - BLOCK_SIZE ) >= GRID_START_X && ( Z_block_state == 2'b10 || Z_block_state == 2'b00 ) && ( board_regs[board_reg_idx - 1][5] == 1'b0 ) 
                 && ( board_regs[board_reg_idx + 10][5] == 1'b0 ) ) begin                               
                BlockX <= BlockX - BLOCK_SIZE;
            end
            
            // Else if D is pressed, block is vertical, and move is in bounds
            else if ( dPressed == 1'b1 && ( BlockX + WIDTH + BLOCK_SIZE ) <= GRID_END_X && ( Z_block_state == 2'b11 || Z_block_state == 2'b01 ) && ( board_regs[board_reg_idx + 2][5] == 1'b0 ) 
            ...
            else if ( dPressed == 1'b1 && ( BlockX + HEIGHT + BLOCK_SIZE ) <= GRID_END_X  && ( Z_block_state == 2'b00 || Z_block_state == 2'b10 ) && ( board_regs[board_reg_idx + 2][5] == 1'b0 ) 
                     && ( board_regs[board_reg_idx + 13][5] == 1'b0 ) ) begin                               
                BlockX <= BlockX + BLOCK_SIZE;
            end

            // Else if S is pressed, block is vertical, and move is in bounds
            else if ( ( sPressed == 1'b1 && ( BlockY + HEIGHT + BLOCK_SIZE <= GRID_END_Y  ) && ( Z_block_state == 2'b11 || Z_block_state == 2'b01 ) ) )  begin                
                if ( ( board_regs[board_reg_idx + 21][5] == 1'b0 ) 
                && ( board_regs[board_reg_idx + 30][5] == 1'b0 ) ) begin 
                    BlockY <= BlockY + BLOCK_SIZE;
                end
                else begin 
                    BlockPlaced <= 1'b1; // if block is placed, need to update board, set addrs
            ...
            else if ( ( sPressed == 1'b1 && ( BlockY + WIDTH + BLOCK_SIZE <= GRID_END_Y  ) && (  Z_block_state == 2'b00 || Z_block_state == 2'b10 ) ) ) begin                
