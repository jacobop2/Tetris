...
//    assign raddr4 = 8'd201;
    ...
    logic L_block_state = 2'b00; //start state of block L up and down
    ...
    always_ff @ ( posedge frame_clk ) 
        ...
        if ( GenerateBlock == 1'b1 ) begin
            ...
            if ( frame_counter == FRAME_COUNTER_MAX ) begin
                ...
                if ( BlockY + BLOCK_SIZE + HEIGHT <= GRID_END_Y && ( L_block_state == 2'b11 ) && ( board_regs[board_reg_idx + 10][5] == 1'b0 ) 
                && ( board_regs[board_reg_idx + 31][5] == 1'b0 ) ) begin 
                    BlockY <= BlockY + BLOCK_SIZE;
                end 
                //in vertical 2
                else if ( ( BlockY + BLOCK_SIZE + HEIGHT <= GRID_END_Y ) && L_block_state == 2'b01 && ( board_regs[board_reg_idx + 30][5] == 1'b0 )
                        && ( board_regs[board_reg_idx + 31][5] == 1'b0 ) ) begin 
                     BlockY <= BlockY + BLOCK_SIZE;
                end
                // If in horizontal state, check that bottom is in bounds
                else if ( BlockY + BLOCK_SIZE + WIDTH <= GRID_END_Y && ( L_block_state == 2'b10 ) && ( board_regs[board_reg_idx + 20][5] == 1'b0 )
                       && ( board_regs[board_reg_idx + 11][5] == 1'b0 ) && ( board_regs[board_reg_idx + 12][5] == 1'b0 ) ) begin                    
                     BlockY <= BlockY + BLOCK_SIZE;
                end
                //in horizontal 2
                else if ( BlockY + BLOCK_SIZE + WIDTH <= GRID_END_Y && ( L_block_state == 2'b00 ) && ( board_regs[board_reg_idx + 20][5] == 1'b0 ) 
        ...
        if ( frame_counter_xmove == FRAME_COUNTER_XMOVE_MAX && ( ( BlockY + HEIGHT + BLOCK_SIZE <= GRID_END_Y && ( L_block_state == 2'b11 || L_block_state == 2'b01 ) ) 
                                                        || ( BlockY + WIDTH + BLOCK_SIZE <= GRID_END_Y && ( L_block_state == 2'b10 || L_block_state == 2'b00 ) ) ) ) begin //10
            
            //***** MOVE *****
            // If A is pressed and that move is in bounds and any state
            if ( aPressed == 1'b1 && ( BlockX - BLOCK_SIZE ) >= GRID_START_X &&  L_block_state == 2'b11 && ( board_regs[board_reg_idx - 1][5] == 1'b0 )
            && ( board_regs[board_reg_idx + 10][5] == 1'b0 ) && ( board_regs[board_reg_idx + 20][5] == 1'b0 ) ) begin                               
                BlockX <= BlockX - BLOCK_SIZE;
            end
            
            else if ( aPressed == 1'b1 && ( BlockX - BLOCK_SIZE ) >= GRID_START_X &&  L_block_state == 2'b10 && ( board_regs[board_reg_idx - 1][5] == 1'b0 ) 
                 && ( board_regs[board_reg_idx + 9][5] == 1'b0 ) ) begin                               
                BlockX <= BlockX - BLOCK_SIZE;
            end
            
            else if ( aPressed == 1'b1 && ( BlockX - BLOCK_SIZE ) >= GRID_START_X &&  L_block_state == 2'b01 && ( board_regs[board_reg_idx - 1][5] == 1'b0 ) 
                 && ( board_regs[board_reg_idx + 9][5] == 1'b0 ) && ( board_regs[board_reg_idx + 19][5] == 1'b0 ) ) begin                               
                BlockX <= BlockX - BLOCK_SIZE;
            end
            
            else if ( aPressed == 1'b1 && ( BlockX - BLOCK_SIZE ) >= GRID_START_X &&  L_block_state == 2'b00 && ( board_regs[board_reg_idx + 1][5] == 1'b0 ) 
                 && ( board_regs[board_reg_idx + 9][5] == 1'b0 ) ) begin                               
                BlockX <= BlockX - BLOCK_SIZE;
            end

            // Else if D is pressed, block is vertical, and move is in bounds
            else if ( dPressed == 1'b1 && ( BlockX + WIDTH + BLOCK_SIZE ) <= GRID_END_X && ( L_block_state == 2'b11 ) && ( board_regs[board_reg_idx + 2][5] == 1'b0 ) 
                     && ( board_regs[board_reg_idx + 22][5] == 1'b0 ) && ( board_regs[board_reg_idx + 12][5] == 1'b0 ) ) begin     // changed from <=, and bottom one                            
                BlockX <= BlockX + BLOCK_SIZE;
            end
            
            else if ( dPressed == 1'b1 && ( BlockX + WIDTH + BLOCK_SIZE ) <= GRID_END_X && ( L_block_state == 2'b01 ) && ( board_regs[board_reg_idx + 1][5] == 1'b0 ) 
                     && ( board_regs[board_reg_idx + 11][5] == 1'b0 ) && ( board_regs[board_reg_idx + 22][5] == 1'b0 ) ) begin     // changed from <=, and bottom one                            
                BlockX <= BlockX + BLOCK_SIZE;
            end
            
            else if ( dPressed == 1'b1 && ( BlockX + HEIGHT + BLOCK_SIZE ) <= GRID_END_X && ( L_block_state == 2'b00 ) && ( board_regs[board_reg_idx + 3][5] == 1'b0 ) 
            ...
            else if ( ( sPressed == 1'b1 && ( BlockY + HEIGHT + BLOCK_SIZE <= GRID_END_Y  ) && ( L_block_state == 2'b11  ) ) )  begin                
                if ( ( board_regs[board_reg_idx + 10][5] == 1'b0 ) 
                && ( board_regs[board_reg_idx + 31][5] == 1'b0 ) ) begin 
                    BlockY <= BlockY + BLOCK_SIZE;
                end
                else begin 
            ...
            else if ( ( sPressed == 1'b1 && ( BlockY + HEIGHT + BLOCK_SIZE <= GRID_END_Y  ) && ( L_block_state == 2'b01 ) ) ) begin                
            ...
            else if ( ( sPressed == 1'b1 && ( BlockY + WIDTH + BLOCK_SIZE <= GRID_END_Y  ) && (  L_block_state == 2'b00 ) ) ) begin                
            ...
            else if ( ( sPressed == 1'b1 && ( BlockY + WIDTH + BLOCK_SIZE <= GRID_END_Y  ) && ( L_block_state == 2'b10  ) ) ) begin                
                if ( ( board_regs[board_reg_idx + 20][5] == 1'b0 )
                       && ( board_regs[board_reg_idx + 11][5] == 1'b0 ) && ( board_regs[board_reg_idx + 12][5] == 1'b0 ) ) begin 
                    BlockY <= BlockY + BLOCK_SIZE;
                end
            ...
            if ( wPressed == 1'b1 ) begin
                ...
                if( L_block_state == 2'b11 ) begin
                    if ( ( BlockX + 3*BLOCK_SIZE ) <= GRID_END_X && ( board_regs[board_reg_idx + 2][5] == 1'b0 )
                       && ( board_regs[board_reg_idx + 12][5] == 1'b0 ) && ( board_regs[board_reg_idx + 10][5] == 1'b0 ) ) begin
                        L_block_state <= 2'b00;
                    end
                end
                
                else if ( L_block_state == 2'b10 ) begin 
                    if ( ( BlockY - BLOCK_SIZE ) >= GRID_START_Y && ( board_regs[board_reg_idx - 10][5] == 1'b0 )
                       && ( board_regs[board_reg_idx + 11][5] == 1'b0 ) && ( board_regs[board_reg_idx - 9][5] == 1'b0 ) ) begin
                        BlockY <= BlockY - BLOCK_SIZE;
                        L_block_state <= 2'b11;
                    end
                end
                    ...
                    if ( ( BlockX - BLOCK_SIZE ) >= GRID_START_X && ( board_regs[board_reg_idx + 9][5] == 1'b0 )
                       && ( board_regs[board_reg_idx + 21][5] == 1'b0 ) && ( board_regs[board_reg_idx + 19][5] == 1'b0 ) ) begin
                        BlockX <= BlockX - BLOCK_SIZE;
                        BlockY <= BlockY + BLOCK_SIZE;
                        L_block_state <= 2'b10;
                    end
                    ...
                    if ( ( BlockY + 3*BLOCK_SIZE ) <= GRID_END_Y && ( board_regs[board_reg_idx + 1][5] == 1'b0 )
                       && ( board_regs[board_reg_idx + 1][5] == 1'b0 ) && ( board_regs[board_reg_idx + 21][5] == 1'b0 ) && ( board_regs[board_reg_idx + 22][5] == 1'b0 ) ) begin
                        BlockX <= BlockX + BLOCK_SIZE;
                        L_block_state <= 2'b01;
                    end
                end
